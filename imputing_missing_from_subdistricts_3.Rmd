...
https://documentation.sas.com/doc/en/pgmsascdc/9.4_3.4/statug/statug_surveyimpute_details27.htm
...
#Now combining the imputed data with the original data
...
draft <- left_join(draft, arr, by = "id")
draft <- draft %>% 
  mutate(wrong_imp = ifelse(conc == conc_imp, 0, New.Arrivals..HH.))

## Putting together the whole imputed information (From ' To and the HH numbers) and see how the imputation is working ??? 

```

Note that in terms of evaluating different imputation strategies I'm only looking at how imputation changes the totals for the "from" sub-districts, not at whether the imputation predictions the "from" subdistrict for each community.
...
#Per community, calculate  wrong imputetd / total imputate (then mean of all %s )
# 
...
Per community, % no to yes

```{r eval=FALSE, include=FALSE}
#Computing the percentage 

c <- comm_na_hotdeck %>%
  group_by(T_Community_En, F_Sub.District_En, original, F_Sub.District_En_imp) %>%
  summarize(n = sum(New.Arrivals..HH.)) %>% 
  mutate(F_Sub.District_En_imp = ifelse(original == "yes", FALSE, F_Sub.District_En_imp)) %>% 
  pivot_wider(names_from = original,
              values_from = n)%>%
...
```{r eval=FALSE, include=FALSE}
cal_comm_hotdeck_sub <- function(order, domain, forced = FALSE) {
  if (forced == TRUE){
     data <- comm_forced
  }else{
...
cal_comm_hotdeck_com <- function(order, domain, forced = FALSE) {
...
# SUBHOTDECK_HH_NULL_NULL <- sapply(1:200, function(i) {cal_hh_hotdeck_sub(order = NULL, 
#                                             domain = NULL)}) %>%
#   unlist() %>%
#   mean()

COMHOTDECK_HH_NULL_NULL <- sapply(1:200, function(i) {cal_hh_hotdeck_com_draft(order = NULL, 
                                            domain = NULL)}) %>%
  unlist() %>%
  mean()
...
set.seed(100)
# 
# SUBHOTDECK_HH_NULL_DIS.ARRIVAL <- sapply(1:200, function(i) {cal_hh_hotdeck_sub(order = NULL, 
#                                             domain = "T_District_En")}) %>%
#   unlist() %>%
#   mean()

COMHOTDECK_HH_NULL_DIS.ARRIVAL <- sapply(1:200, function(i) {cal_hh_hotdeck_com_draft(order = NULL, 
                                            domain = "T_District_En")}) %>%
  unlist() %>%
  mean()
...
# SUBHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL <- sapply(1:200, function(i) {cal_hh_hotdeck_sub(order = "New.Arrivals..HH.", 
#                                             domain = "T_District_En")}) %>%
#   unlist() %>%
#   mean()

COMHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL <- sapply(1:200, function(i) {cal_hh_hotdeck_com_draft(order = "New.Arrivals..HH.", 
                                            domain = "T_District_En")}) %>%
  unlist() %>%
  mean()
...
#Codes added to export results for paper
#Creating data frame with all results
# plot4sub <- data.frame(Approach=c("SUBHOTDECK_HH_NULL_NULL","SUBHOTDECK_HH_NULL_DIS.ARRIVAL","SUBHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL"),
#                        Margin_Error = c(SUBHOTDECK_HH_NULL_NULL,SUBHOTDECK_HH_NULL_DIS.ARRIVAL,SUBHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL)) %>% 
#   mutate(Margin_Error = round(Margin_Error, 4))

plot4com_draft <- data.frame(Approach=c("COMHOTDECK_HH_NULL_NULL","COMHOTDECK_HH_NULL_DIS.ARRIVAL","COMHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL"), 
                       Margin_Error = c(COMHOTDECK_HH_NULL_NULL,COMHOTDECK_HH_NULL_DIS.ARRIVAL,COMHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL)) %>% 
  mutate(Margin_Error = round(Margin_Error, 4))

# write_xlsx(plot4sub, "output/plot4sub.xlsx")
write_xlsx(plot4com_draft, "output/plot4com_draft.xlsx")
```
...
# Forced random to areas were NA is distributed in Decemebr 2020
```{r eval=FALSE, include=FALSE}
set.seed(100)

# SUBHOTDECK_COM_NULL_NULL_F <- sapply(1:200, function(i) {cal_comm_hotdeck_sub(order = NULL, 
#                                             domain = NULL, forced = TRUE)}) %>%
#   unlist() %>%
#   mean()

COMHOTDECK_COM_NULL_NULL_F <- sapply(1:200, function(i) {cal_comm_hotdeck_com_draft(order = NULL,
                                                                            domain = NULL,
                                                                            forced = TRUE)}) %>% 
  unlist() %>% 
  mean()
...
# SUBHOTDECK_COM_NULL_DIS.ARRIVAL_F <- sapply(1:200, function(i) {cal_comm_hotdeck_sub(order = NULL, 
#                                             domain = "T_District_En",forced = TRUE)}) %>%
#   unlist() %>%
#   mean()

COMHOTDECK_COM_NULL_DIS.ARRIVAL_F <- sapply(1:200, function(i) {cal_comm_hotdeck_com_draft(order = NULL, 
                                            domain = "T_District_En",forced = TRUE)}) %>%
  unlist() %>%
  mean()
...
# SUBHOTDECK_COM_HH.ARRIVAL_DIS.ARRIVAL_F <- sapply(1:200, function(i) {cal_comm_hotdeck_sub(order = "New.Arrivals..HH.", 
#                                             domain = "T_District_En",forced = TRUE)}) %>%
#   unlist() %>%
#   mean()

COMHOTDECK_COM_HH.ARRIVAL_DIS.ARRIVAL_F <- sapply(1:200, function(i) {cal_comm_hotdeck_com_draft(order = "New.Arrivals..HH.", 
                                            domain = "T_District_En",forced = TRUE)}) %>%
  unlist() %>%
  mean()
...
# SUBkNN_COM_100_LAT.LON_SAMPLE_TRUE_F <- sapply(1:200, function(i) {cal_comm_knn_sub(100, dist_var = c("lat", "long"),
#                                                                                   catFun = sampleCat, addRandom = TRUE,
#                                                                                   forced = TRUE)}) %>%
#   unlist() %>%
#   mean()

COMkNN_COM_100_LAT.LON_SAMPLE_TRUE_F <- sapply(1:200, function(i) {cal_comm_knn_com_draft(100, dist_var = c("lat", "long"),
                                                                                  catFun = sampleCat, addRandom = TRUE,
                                                                                  forced = TRUE)}) %>%
  unlist() %>%
  mean()
...
# SUBkNN_HH_100_LAT.LON_SAMPLE_TRUE_F <- sapply(1:100, function(i) {cal_hh_knn_sub(100, dist_var = c("lat", "long"),
#                                                                                catFun = sampleCat, addRandom = TRUE,
#                                                                                forced = TRUE)}) %>%
#   unlist() %>%
#   mean()

COMkNN_HH_100_LAT.LON_SAMPLE_TRUE_F <- sapply(1:100, function(i) {cal_hh_knn_com_draft(100, dist_var = c("lat", "long"),
                                                                               catFun = sampleCat, addRandom = TRUE,
                                                                               forced = TRUE)}) %>%
  unlist() %>%
  mean()
...
# SUBkNN_HH_50_LAT.LON_SAMPLE_TRUE_F <- sapply(1:100, function(i) {cal_hh_knn_sub(50, dist_var = c("lat", "long"),
#                                                                               catFun = sampleCat, addRandom = TRUE,
#                                                                               forced = TRUE)}) %>%
#   unlist() %>%
#   mean()

COMkNN_HH_50_LAT.LON_SAMPLE_TRUE_F <- sapply(1:100, function(i) {cal_hh_knn_com_draft(50, dist_var = c("lat", "long"),
                                                                              catFun = sampleCat, addRandom = TRUE,
                                                                              forced = TRUE)}) %>%
  unlist() %>%
  mean()
...
# SUBkNN_HH_200_LAT.LON_SAMPLE_TRUE_F <- sapply(1:100, function(i) {cal_hh_knn_sub(200, dist_var = c("lat", "long"),
#                                                                                catFun = sampleCat, addRandom = TRUE,
#                                                                                forced = TRUE)}) %>%
#   unlist() %>%
#   mean()

COMkNN_HH_200_LAT.LON_SAMPLE_TRUE_F <- sapply(1:100, function(i) {cal_hh_knn_com_draft(200, dist_var = c("lat", "long"),
                                                                               catFun = sampleCat, addRandom = TRUE,
                                                                               forced = TRUE)}) %>%
  unlist() %>%
  mean()
...
# SUBHOTDECK_HH_NULL_NULL_F <- sapply(1:200, function(i) {cal_hh_hotdeck_sub(order = NULL, 
#                                             domain = NULL,forced = TRUE)}) %>%
#   unlist() %>%
#   mean()

COMHOTDECK_HH_NULL_NULL_F <- sapply(1:200, function(i) {cal_hh_hotdeck_com_draft(order = NULL, 
                                            domain = NULL,forced = TRUE)}) %>%
  unlist() %>%
  mean()
...
# SUBHOTDECK_HH_NULL_DIS.ARRIVAL_F <- sapply(1:200, function(i) {cal_hh_hotdeck_sub(order = NULL, 
#                                             domain = "T_District_En",forced = TRUE)}) %>%
#   unlist() %>%
#   mean()

COMHOTDECK_HH_NULL_DIS.ARRIVAL_F <- sapply(1:200, function(i) {cal_hh_hotdeck_com_draft(order = NULL, 
                                            domain = "T_District_En",forced = TRUE)}) %>%
  unlist() %>%
  mean()
...
# SUBHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL_F <- sapply(1:200, function(i) {cal_hh_hotdeck_sub_draft(order = "New.Arrivals..HH.", 
#                                             domain = "T_District_En",forced = TRUE)}) %>%
#   unlist() %>%
#   mean()

COMHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL_F <- sapply(1:200, function(i) {cal_hh_hotdeck_com_draft(order = "New.Arrivals..HH.", 
                                            domain = "T_District_En",forced = TRUE)}) %>%
  unlist() %>%
  mean()
```
...
```{r}
# plot1subforced <- data.frame(Approach=c("SUBHOTDECK_COM_NULL_NULL","SUBHOTDECK_COM_NULL_DIS.ARRIVAL",
#                                   "SUBHOTDECK_COM_HH.ARRIVAL_DIS.ARRIVAL","SUBkNN_COM_100_LAT.LON_SAMPLE_TRUE"),
#                        Margin_Error = c(SUBHOTDECK_COM_NULL_NULL,SUBHOTDECK_COM_NULL_DIS.ARRIVAL,
#                                         SUBHOTDECK_COM_HH.ARRIVAL_DIS.ARRIVAL,SUBkNN_COM_100_LAT.LON_SAMPLE_TRUE)) %>% 
#   mutate(Margin_Error = round(Margin_Error, 4))

plot1com_draft_f <- data.frame(Approach=c("COMHOTDECK_COM_NULL_NULL_F","COMHOTDECK_COM_NULL_DIS.ARRIVAL_F",
                                  "COMHOTDECK_COM_HH.ARRIVAL_DIS.ARRIVAL_F","COMkNN_COM_100_LAT.LON_SAMPLE_TRUE_F"),
                       Margin_Error = c(COMHOTDECK_COM_NULL_NULL_F,COMHOTDECK_COM_NULL_DIS.ARRIVAL_F,
                                        COMHOTDECK_COM_HH.ARRIVAL_DIS.ARRIVAL_F,COMkNN_COM_100_LAT.LON_SAMPLE_TRUE_F)) %>% 
  mutate(Margin_Error = round(Margin_Error, 4))


# plot2subforced <- data.frame(Approach=c("SUBkNN_HH_50_LAT.LON_SAMPLE_TRUE","SUBkNN_HH_100_LAT.LON_SAMPLE_TRUE",
#                                   "SUBkNN_HH_200_LAT.LON_SAMPLE_TRUE"),
#                        Margin_Error = c(SUBkNN_HH_50_LAT.LON_SAMPLE_TRUE,SUBkNN_HH_100_LAT.LON_SAMPLE_TRUE,
#                                         SUBkNN_HH_200_LAT.LON_SAMPLE_TRUE)) %>% 
#   mutate(Margin_Error = round(Margin_Error, 4))

plot2com_draft_f <- data.frame(Approach=c("COMkNN_HH_50_LAT.LON_SAMPLE_TRUE_F","COMkNN_HH_100_LAT.LON_SAMPLE_TRUE_F",
                                  "COMkNN_HH_200_LAT.LON_SAMPLE_TRUE_F"),
                       Margin_Error = c(COMkNN_HH_50_LAT.LON_SAMPLE_TRUE_F,COMkNN_HH_100_LAT.LON_SAMPLE_TRUE_F,COMkNN_HH_200_LAT.LON_SAMPLE_TRUE_F)) %>% 
  mutate(Margin_Error = round(Margin_Error, 4))

# plot3subforced <- data.frame(Approach=c("SUBkNN_HH_200_LAT.LON.HH_SAMPLE_TRUE","SUBkNN_HH_200_LAT.LON_MAX_TRUE",
#                                "SUBkNN_HH_200_LAT.LON_SAMPLE_FALSE"),
#                     Margin_Error = c(SUBkNN_HH_200_LAT.LON.HH_SAMPLE_TRUE,SUBkNN_HH_200_LAT.LON_MAX_TRUE,
#                                      SUBkNN_HH_200_LAT.LON_SAMPLE_FALSE)) %>% 
#   mutate(Margin_Error = round(Margin_Error, 4))
...
# plot4subforced <- data.frame(Approach=c("SUBHOTDECK_HH_NULL_NULL","SUBHOTDECK_HH_NULL_DIS.ARRIVAL","SUBHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL"),
#                        Margin_Error = c(SUBHOTDECK_HH_NULL_NULL,SUBHOTDECK_HH_NULL_DIS.ARRIVAL,SUBHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL)) %>% 
#   mutate(Margin_Error = round(Margin_Error, 4))

plot4com_draft_f <- data.frame(Approach=c("COMHOTDECK_HH_NULL_NULL_F","COMHOTDECK_HH_NULL_DIS.ARRIVAL_F",
                                          "COMHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL_F"), 
                       Margin_Error = c(COMHOTDECK_HH_NULL_NULL_F,COMHOTDECK_HH_NULL_DIS.ARRIVAL_F
                                        ,COMHOTDECK_HH_HH.ARRIVAL_DIS.ARRIVAL_F)) %>% 
  mutate(Margin_Error = round(Margin_Error, 4))

# write_xlsx(plot1subforced, "output/plot1subforced.xlsx")
write_xlsx(plot1com_draft_f, "output/plot1com_draft_f.xlsx")
# write_xlsx(plot2subforced, "output/plot2subforced.xlsx")
write_xlsx(plot2com_draft_f, "output/plot2com_draft_f.xlsx")
# write_xlsx(plot3subforced, "output/plot3subforced.xlsx")
write_xlsx(plot3com_draft_f, "output/plot3com_draft_f.xlsx")
# write_xlsx(plot4subforced, "output/plot4subforced.xlsx")
write_xlsx(plot4com_draft_f, "output/plot4com_draft_f.xlsx")
```
