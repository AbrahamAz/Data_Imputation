...
library(tidyverse)
library(shiny)
data <- readxl::read_xlsx("unknown_LPD.xlsx") %>% 
...
<li> Forced selection of NA values on .....</li></ul>
...
library(readxl)
plot1 <- read_excel("src/plot1.xlsx")
...
```{r echo=FALSE, warning=FALSE}
plot2 <- read_excel("src/plot2.xlsx")
...
```{r echo=FALSE, warning=FALSE}
plot3 <- read_excel("src/plot3.xlsx")
...
```{r echo=FALSE, warning=FALSE}
plot4 <- read_excel("src/plot4.xlsx")
...
library(leaflet)
library(sf)
gps_na <- readxl::read_excel("Consolidated_Datasets.xlsx")
...
gps_na_dec2020 <- gps_na %>% 
  filter(`Number of IDP HH arrivals` != 0 & `Start of data coverage peirod` == "December 2020")

from_To <- gps_na_dec2020 %>% 
  select(`Closest p-code`,`Number of IDP HH arrivals`, `Subdistrict of last departure 1`, `Number of IDP HHs arrived from subdistrict 1`,
...
from_To.cleaned$`Number of IDP HHs arrived from subdistrict 1` <- as.numeric(from_To.cleaned$`Number of IDP HHs arrived from subdistrict 1`)
from_To.cleaned$`Number of IDP HHs arrived from subdistrict 2` <- as.numeric(from_To.cleaned$`Number of IDP HHs arrived from subdistrict 2`)
from_To.cleaned$`Number of IDP HHs arrived from subdistrict 3` <- as.numeric(from_To.cleaned$`Number of IDP HHs arrived from subdistrict 3`)

from_To.cleaned <- from_To.cleaned %>% 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
inputPanel(
  selectInput("date", label = "Select Month", choices = unique(gps_na$`Start of data coverage peirod`), selected = "September 2021")
)
```
...
filteredData <- reactive({
  req(input$date)
  gps_na[gps_na$`Start of data coverage peirod` == input$date,]
})

observe({
  from_To <- filteredData() %>% 
  select(`Closest p-code`,`Number of IDP HH arrivals`, `Subdistrict of last departure 1`, `Number of IDP HHs arrived from subdistrict 1`,
         `Subdistrict of last departure 2`, `Number of IDP HHs arrived from subdistrict 2`,
         `Subdistrict of last departure 3`, `Number of IDP HHs arrived from subdistrict 3`) %>% 
  rename("To_Com" = `Closest p-code`,
...
<b>Best simulation</b> was the kNN, HH level, 200 NNs sampled proportionately with random noise added. 
