...
For both, randomized and forced selection of NAs, we used a full dataset from ISMI (without NAs) and generated NAs entries in the *From Sub-district* field. We also gave unique IDs for each entry to separate the data that will be used for imputation studies in the simulations to calculate the error rate. We compared the result by calculating the wrong imputed number of household arrivals for each *movement entry* and dividing it by the total number of household arrivals of all imputed data.
...
- Each plot in both tabs represent different level of imputation on which the error rate is calculated (community and household levels). All the plots represent the aggregation of the data on community level. <br><br>
...
ggplot(plot1com_draft, aes(x=Approach, y=Margin_Error, label = Margin_Error), width= 50)+
...
ggplot(plot2com_draft, aes(x=Approach, y=Margin_Error, label = Margin_Error), width= 50)+
  geom_col(fill = "#ee5859", width = 0.2,position = position_dodge(width=0.1), aes(ylab = Margin_Error)) +
  theme(axis.text.x = element_text(vjust = 0.5,angle = 25)) +
  geom_text(nudge_y = 0.03)
```
...
ggplot(plot3com_draft, aes(x=Approach, y=Margin_Error, label = Margin_Error), width= 50)+
  geom_col(fill = "#ee5859", width = 0.2,position = position_dodge(width=0.1), aes(ylab = Margin_Error)) +
  theme(axis.text.x = element_text(vjust = 0.5,angle = 25)) +
  geom_text(nudge_y = 0.03)
```
...
ggplot(plot4com_draft, aes(x=Approach, y=Margin_Error, label = Margin_Error), width= 50)+
  geom_col(fill = "#ee5859", width = 0.2,position = position_dodge(width=0.1), aes(ylab = Margin_Error)) +
  theme(axis.text.x = element_text(vjust = 0.5,angle = 25)) +
  geom_text(nudge_y = 0.03)
```
...
leaflet() %>%
  ...
  addCircles(data = unknown_com.sf, stroke = F, fill= T, color = "#ee5859",
             # label = unknown_com.sf$enumID, labelOptions = labelOptions(noHide = T),
             fillOpacity = 0.75,
...
ggplot(plot1com_draft_F, aes(x=Approach, y=Margin_Error, label = Margin_Error), width= 50)+
  geom_col(fill = "#ee5859", width = 0.2,position = position_dodge(width=0.1), aes(ylab = Margin_Error)) +
  theme(axis.text.x = element_text(vjust = 0.5,angle = 25)) +
  geom_text(nudge_y = 0.03)

plot2com_draft_F <- read_excel("output/plot2com_draft_F.xlsx")
ggplot(plot2com_draft_F, aes(x=Approach, y=Margin_Error, label = Margin_Error), width= 50)+
  geom_col(fill = "#ee5859", width = 0.2,position = position_dodge(width=0.1), aes(ylab = Margin_Error)) +
  theme(axis.text.x = element_text(vjust = 0.5,angle = 25)) +
  geom_text(nudge_y = 0.03)

plot3com_draft_F <- read_excel("output/plot3com_draft_F.xlsx")
ggplot(plot3com_draft_F, aes(x=Approach, y=Margin_Error, label = Margin_Error), width= 50)+
  geom_col(fill = "#ee5859", width = 0.2,position = position_dodge(width=0.1), aes(ylab = Margin_Error)) +
  theme(axis.text.x = element_text(vjust = 0.5,angle = 25)) +
  geom_text(nudge_y = 0.03)

plot4com_draft_F <- read_excel("output/plot4com_draft_F.xlsx")
ggplot(plot4com_draft_F, aes(x=Approach, y=Margin_Error, label = Margin_Error), width= 50)+
  geom_col(fill = "#ee5859", width = 0.2,position = position_dodge(width=0.1), aes(ylab = Margin_Error)) +
  theme(axis.text.x = element_text(vjust = 0.5,angle = 25)) +
  geom_text(nudge_y = 0.03)
```
...
Taking into consideration the previous results: <br>
Bother Hot Deck and kNN are not applicaple for this case because the results with high error rate is not a good sign. *UNTIL PROVEN WRONG*. <br>
Looking at other imputation methodologies.
